...
def get_pop_description(rawdataframe,hhcat,industry_list,listofdeciles):
	...
	hhdataframe.rename(columns={'totwgt':'totweight','wgthh2007':'weight'},inplace=True)
	hhdataframe['nbpeople']=hhdataframe['totweight']/hhdataframe['weight']
	int_columns=['idh','Y','totY','children','old','decile']+['cat{}workers'.format(thecat) for thecat in hhcat['hhcat'].unique()]+['nbpeople','weight']
	return hhdataframe[int_columns]
...
def create_correct_data(countrycode,data_gidd_csv,hhcat,industry_list):
